...
def predict():
    ...
    with torch.no_grad():
        if request.method == 'POST':
            ...
            A_img.save("ActualImg.jpeg")
            print(f'### Actual Img type is : {type(A_img)}')
            from torchvision import transforms

            transform = transforms.Compose([
                    transforms.Resize((28, 28), interpolation=0),
                    transforms.ToTensor(),
                    transforms.Normalize((0.5), (0.5))
                ])

            img_t = transform(A_img)
            device = 'cuda' if torch.cuda.is_available() else 'cpu'
            #img_t = img_t.to(device)
            print(f'### Tensor Img shape is : {img_t.shape}')
            my_prediction = model(img_t.unsqueeze(0)).detach().numpy().argmax()
